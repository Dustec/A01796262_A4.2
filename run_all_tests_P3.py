"""
Script para ejecutar todos los casos de prueba del programa P3.
...
def ensure_directories():
    ...
    os.makedirs("results/P3", exist_ok=True)
...
def run_test_case(test_number):
    ...
    Args:
        test_number (int): Número del test case

    Returns:
        tuple: (success, elapsed_time, output)
    """
    input_file = f"tests/P3/TC{test_number}.txt"
    output_file = f"results/P3/TC{test_number}_results.txt"
...
def generate_consolidated_report(results):
    ...
    """
    consolidated_file = "results/P3/WordCountResults.txt"

    try:
        with open(consolidated_file, 'w', encoding='utf-8') as f:
            f.write("=" * 80 + "\n")
            f.write("REPORTE CONSOLIDADO - PROGRAMA P3: wordCount.py\n")
...
def main():
    """Función principal."""
    print("=" * 80)
    print("EJECUTOR DE PRUEBAS - PROGRAMA P3: wordCount.py")
    print("=" * 80)
    ...
    # Detectar cuántos test cases hay
    test_files = []
    for i in range(1, 20):  # Buscar hasta TC20
        test_file = f"tests/P3/TC{i}.txt"
        if os.path.exists(test_file):
            test_files.append(i)
        else:
            break  # Detenerse cuando no encuentre más archivos

    if not test_files:
        print("\n✗ No se encontraron archivos de test en tests/P3/")
        return 1

    print(f"\nTest cases encontrados: {len(test_files)}")

    # Ejecutar todos los test cases encontrados
    for i in test_files:
